...
/**
 * Class CmsPluginFunctionsUmbraco
 */
class CmsPluginFunctionsUmbraco
{
    ...
    private $jsConfig = '{
        ...
        "wp_feedback"  : "//wordpress.org/support/plugin/cms2cms-umbraco-to-wp-migrator/reviews",
    ...
    {
        $cms2cms_umbraco_access_login = get_option('cms2cms-umbraco-login');
        $cms2cms_umbraco_access_key   = get_option('cms2cms-umbraco-key');

        return array(
            'email'     => $cms2cms_umbraco_access_login,
            'accessKey' => $cms2cms_umbraco_access_key
        );
    }
    ...
    public function isActivated()
    {
        ...
        return ($cms2cms_umbraco_access_key != false);
    }
    ...
    public function getOptions()
    {
        ...
        $login = get_option('cms2cms-umbraco-login');
    ...
    public function saveOptions()
    {
        ...
        if ( $key && $login ) {
            ...
            add_option('cms2cms-umbraco-key', $key);

            delete_option('cms2cms-umbraco-login');
            add_option('cms2cms-umbraco-login', $login);

            delete_option('cms2cms-umbraco-depth');
            add_option('cms2cms-umbraco-depth', $bridge_depth);
    ...
    {
        return $this->__('Umbraco', 'cms2cms-umbraco-migration');
    }
    ...
    {
        return $this->__('WordPress', 'cms2cms-umbraco-migration');
    }
    ...
    {
        return sprintf(
            $this->__('CMS2CMS: Automated %s to %s Migration ', 'cms2cms-umbraco-migration'),
    ...
    {
        return sprintf(
            $this->__('%s to %s', 'cms2cms-umbraco-migration'),
            $this->getPluginSourceName(),
            $this->getPluginTargetName()
        );
    }
    ...
    public function logOut()
    {
        if (isset($_REQUEST['_wpnonce'])) {
            ...
            if ($this->verifyFormTempKey($nonce, 'cms2cms_umbraco_logout')
                && $_POST['cms2cms_umbraco_logout'] == 1
            ) {
                $this->clearOptions();
            }
        }
